...
1. Bump the version in **three** places, which must stay in step:
   ...
   - `src/version.ts`, which feeds the User-Agent sent to LRCLIB
...
```bash
npm view mcp-lrclib version
npx -y mcp-lrclib          # should start and wait on stdin
```
