...
Open an issue: <https://github.com/smeet666/mcp-lrclib/issues>
...
a track whose synced lyrics come back with the wrong timings is a real bug, and it is only visible to
...
It reads LRCLIB and returns what it reads. It does not write anything back, it
...
- **Every result carries its source.** Content that came from LRCLIB goes out
  with a link to the page it came from, and the text block keeps that link even
  when it has to be shortened.
- **The server paces itself.** LRCLIB is someone else's site. The minimum
...
`LRCLIB_LIVE=1 npm run test:live`, which does: it makes one request per route
against https://lrclib.net and checks the fields the parsers depend on. Run it if you
...
The tools are `search_tracks`, `get_track` and `get_lyrics`. A new tool is a bigger conversation than a new field on
